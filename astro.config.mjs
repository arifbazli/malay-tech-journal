...
export default defineConfig({
  ...
  integrations: [
    ...
    expressiveCode({
      ...
      // Bind the active theme to our `<html data-theme="chirpy-dark">`
  ...
  fonts: [
    // Inter — primary UI body font from @fontsource/inter npm package
    // (also used by src/utils/og-image.ts for OG image generation).
    //
    // Weights loaded: 400 (body), 500 (UI labels, eyebrows, navigation),
    // 600 (buttons, card meta, medium emphasis), 700 (rare emphasis —
    // most headings use Space Grotesk instead).
    {
      name: 'Inter',
    ...
    // Space Grotesk — display/heading font from @fontsource/space-grotesk npm package.
    // Preloaded: used above the fold for nav-title, bento titles, featured card titles.
    //
    // Weights loaded: 400 (light display, large hero text), 500 (default headings),
    // 600 (sub-headings, featured titles), 700 (rare — display + accents only).
    {
      name: 'Space Grotesk',
    ...
    // Used for: code blocks, inline code, kbd badges, dates, progress bars,
    // pillar metrics, project stacks, read-time.
    //
    // Weights loaded: 400 (regular code), 500 (UI mono labels — slight
    // weight gain keeps metric values legible against the muted body),
    // 600 (emphasis — kbd primary badge, metric values).
    {
      name: 'JetBrains Mono',
