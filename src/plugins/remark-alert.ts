/**
 * remark-alert — converts fenced code blocks with `lang="alert"` to
 * `mtj-alert` HTML.
 *
 * Replaces daisyUI's `.alert-*` classes with our own `.mtj-alert-*`
 * classes. All colors come from the `--mtj-*` design tokens. No
 * daisyUI dependency.
 *
 ...
 *   class:       Extra Tailwind / custom classes to append
 *
...
function buildAlertHtml(opts: AlertOptions): string {
  // ── CSS classes ──────────────────────────────────────────────────────────
  // Use our own .mtj-alert-* classes (NOT daisyUI .alert-*).
  // All styling lives in src/styles/global.css under "ALERT BOXES".
  const classes: string[] = ['mtj-alert'];

  if (opts.type) classes.push(`mtj-alert--${opts.type}`);
  if (opts.style) classes.push(`mtj-alert--${opts.style}`);

  const hasTitle = Boolean(opts.title);

  switch (opts.direction) {
    case 'vertical':
      classes.push('mtj-alert--vertical');
      break;
    case 'horizontal':
      classes.push('mtj-alert--horizontal');
      break;
    case 'responsive':
      classes.push('mtj-alert--vertical', 'sm:mtj-alert--horizontal');
      break;
    default:
      ...
      if (hasTitle) classes.push('mtj-alert--vertical', 'sm:mtj-alert--horizontal');
      break;
  }
  ...
  if (hasTitle) {
    // Title + optional description — nested div structure. Override
    // .prose-chirpy h3 margins with inline styles so heading spacing
    // from the prose theme does not bleed into the alert box.
