...
/**
 * Article posts only (excludes series-index pages).
 * Use this for main listing pagination so series-index entries
 * don't steal slots or distort page counts.
 */
export async function getArticles(locale: Locale): Promise<Post[]> {
  const all = await getPosts(locale);
  return all.filter((p) => p.data.postType !== 'series-index');
}

/**
 * Series-index posts only (postType === 'series-index').
 * Sorted by pubDate desc (most recently updated series first).
 * Fetch these separately and pass to components — never paginate them.
 */
export async function getSeriesIndex(locale: Locale): Promise<Post[]> {
  const all = await getPosts(locale);
  return all.filter((p) => p.data.postType === 'series-index');
}
...
/**
 * Series (categories) with article counts — used for the sidebar
 * "Siri Aktif" widget with progress bars.
...
export function groupByYearMonth(
  ...
  const lang = locale === 'ms' ? 'ms-MY' : 'en-US';
